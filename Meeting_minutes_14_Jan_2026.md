# Meeting Minutes 14-Jan-2026
 No | Action item | Owner | Status |
-----| ------------| ------|----------
| 1 |Give a Demo of RAG example  | Everyone| 4 Members has to go through the youtube and implement RAG correctly|
| 2 |Select an automation tool from the below list or any other tool which you like | Everyone|    - IN progress|
| 3 |Develop or identify website for testing | Everyone|    - IN progress|
| 4 |Write a python script to crawl any website and build a transition graph | Everyone|    - IN progress|
| 5 |Generate Test cases from the State Transition Graph created  through crawling | Everyone|   Done|



![Overall Flow Chart ](https://github.com/girishsukukumar/GEC-TCR-2025-INTERNSHIP/blob/main/images/flow_chart_1.png)



# Tools for Automation 
 |No | Tool Name  | Comments |
|-----| ------------| ------ |
|1|   Selenium |  Free & support python |
|2|  Playwright | Free & support python |
|3|  Appium     | Free &  support python  |
|4| Robot Framework |Free and support python |
|5| Gauge      |Free & Support Python |
